...
public class ipa3 {
    public static void main(String[] args) {
        ...
        String givenLoc = sc.nextLine();
        String giveninstituationName = sc.nextLine();

        int sum = FindNumClearancedByLoc(arr, givenLoc);
        if(sum==0){
            System.out.println("There are no cleared students from this location");
        }
        else{
            System.out.println(sum);
        }

        Institution answer = UpdateInstitutionGrade(giveninstituationName, arr);
        if(answer == null){
            System.out.println("No Institute is available with the specified name");
        }
        else{
            System.out.println(answer.getInstituationName()+"::"+answer.getGrade());
        }
        
    }

    public static int FindNumClearancedByLoc(Institution[] arr, String givenLoc){
        int sum = 0;
        for(int i=0;i<arr.length;i++){
            if(arr[i].getLocation().equalsIgnoreCase(givenLoc)){
    ...
    public static Institution UpdateInstitutionGrade(String giveninstituationName, Institution[] arr){
        String grade;
        for(int i=0;i<arr.length;i++){
            if(arr[i].getInstituationName().equalsIgnoreCase(giveninstituationName)){
                double rating = (arr[i].getNoOfStudentsPlaced() * 100) / arr[i].getNoOfStudentsCleared();
                if(rating >= 80){
                    grade = "A";
                } else {
                    grade = "B";
...
class Institution{
    ...
    Institution(int institutionId, String instituationName, int noOfStudentsPlaced, int noOfStudentsCleared, String location){
